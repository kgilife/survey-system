  ...
  return <div className="product-preview" aria-label="問卷編輯器示意">
    ...
    <div className="preview-body">
      ...
      <div className="preview-canvas">
        <div className="preview-title"><small>顧客體驗調查　·　編輯中</small><strong>{v2 ? "2026 年顧客體驗調查" : "讓每一個問題都有目的。"}</strong></div>
...
const GUIDE_STEPS = {
  ...
  "連結型矩陣題": ["先到「使用者設定」建立填寫者帳號。", "新增「連結型矩陣題」，從 Excel 貼上使用者 ID、顯示項目兩欄。", "新增固定問項並設定簡答／單選／複選題型及選項，再選擇條列或矩陣顯示；預設條列最適合手機。", "等候自動儲存，再用不同帳號登入確認只看到自己獲配的項目。"],
};
...
function sampleQuestion(name, type, summary) {
  ...
  if (type === "linked_matrix") Object.assign(base, { options:[option("site-a","台北據點"),option("site-b","桃園據點")], config:{displayStyle:"list",prompts:[{id:"service",label:"服務品質",type:"single",options:[option("good","良好"),option("average","普通"),option("improve","需要改善")]},{id:"note",label:"補充說明",type:"short",config:{placeholder:"請輸入說明"}}]} });
  return base;
}
...
function BasicPreview({ q, value, onChange }) {
  ...
  if (q.type === "linked_matrix") { const vals=value&&typeof value==="object"?value:{}; const setCell=(itemId,promptId,next)=>onChange({...vals,[itemId]:{...vals[itemId],[promptId]:next}}); return <div className="linked-matrix-list landing-linked-matrix">{q.options.map((o,itemIndex)=><section className="linked-matrix-item" key={o.value}><header><span className="linked-matrix-item-number">{itemIndex+1}</span><h4>{o.label}</h4></header><div className="linked-matrix-item-fields">{q.config.prompts.map((p,promptIndex)=>{const cell=vals[o.value]?.[p.id];return <fieldset className="linked-matrix-list-field" key={p.id}><legend><span className="linked-matrix-question-number">Q{promptIndex+1}.</span> {p.label}<span className="required-mark"> *</span></legend>{p.type==="single"?<div className="matrix-cell-options">{p.options.map(choice=><label className="check" key={choice.value}><input type="radio" name={`landing-${o.value}-${p.id}`} checked={cell===choice.value} onChange={()=>setCell(o.value,p.id,choice.value)}/><span>{choice.label}</span></label>)}</div>:<input className="input" placeholder={p.config?.placeholder||""} value={typeof cell==="string"?cell:""} onChange={e=>setCell(o.value,p.id,e.target.value)}/>}</fieldset>})}</div></section>)}</div>; }
