...
function formatAdminPasswordColumn_(sheet) {
  var headers=sheet.getRange(1,1,1,sheet.getLastColumn()).getValues()[0].map(String),column=headers.indexOf('password')+1;
  if(column>0)sheet.getRange(2,column,Math.max(sheet.getMaxRows()-1,1),1).setNumberFormat('@');
}

/** 部署後需要調整資料表結構時手動執行一次；日常 API 不做結構掃描。 */
function migrateSystem() {
  return withLock_(function() {
    var props=PropertiesService.getScriptProperties(),ss=master_();
